...
for i = 1:numel(genesToCheck)
    ...
    if ~isempty(delRxns)
        ...
        if sol.stat == -1 || isempty(sol.x)         %%  -1 = no feasible solution found
            essentialGenes = [essentialGenes; genesToCheck(i)];
        end
    end
end

[~, essentialGenesIndexes] = ismember(essentialGenes,model.genes);  %% Gir en boolean liste av 0/1 for alle gener om de er essensielle eller ikke
end
