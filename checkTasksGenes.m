...
for i=1:numel(taskStructure)
 ...
 %%Now there are a lot of constraints set according to the task structure, for task i.
 
    %Solve and print
    sol=solveLP(tModel);
    if ~isempty(sol.x)      %% If feasible, move on
        if ~taskStructure(i).shouldFail
            taskReport.ok(i,1)=true;                %% If it is not shouldFail, then it should PASS 
